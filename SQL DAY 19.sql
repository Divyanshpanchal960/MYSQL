...
select * from employeesales;
select * from (
select *, dense_rank() over(order by saleamount) as total_rank  from employeesales)as abc where total_rank=1; -- lowest amount 2nd lowest 3rd lowest and so on 
select * from (
select *,dense_rank() over(order by saleamount desc) as total_rank from employeesales) as abc where total_rank=1; -- highest amont 2nd highest 3rd highenst and so on
select max(saleamount) from employeesales;

-- CTE (COMMAN TABLE EXPRESSION) 
-- EXPRESSION (NAME ON TEMPROARY BASIC)
USE WINDOW_TEST;
SELECT * FROM EMPLOYEE_SALES;
WITH DEPT_TOTAL AS(
SELECT EMP_NAME,SALE_AMOUNT,DEPARTMENT,SUM(SALE_AMOUNT)
 OVER (PARTITION BY DEPARTMENT ORDER BY EMP_ID),
 RANK() OVER(ORDER BY SALE_AMOUNT) AS TOTAL_RANK FROM EMPLOYEE_SALES)
 SELECT SUM(SALE_AMOUNT) FROM EMPLOYEE_SALES WHERE DEPARTMENT='CLOTHING';
 
 SELECT EMP_NAME,SALE_AMOUNT,DEPARTMENT,SUM(SALE_AMOUNT)
 OVER (PARTITION BY EMP_NAME ORDER BY SALE_AMOUNT DESC),
DENSE_RANK() OVER(PARTITION BY EMP_NAME ORDER BY SALE_AMOUNT) AS TOTAL_RANK FROM EMPLOYEE_SALES;
 
 SELECT DEPARTMENT,EMP_NAME ,SUM(SALE_AMOUNT) OVER(PARTITION BY DEPARTMENT ORDER BY SALE_AMOUNT) FROM EMPLOYEE_SALES;
...
select max(saleamount) from employeesales where saleamount<(select max(saleamount) from employeesales)));
